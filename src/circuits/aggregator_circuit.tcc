...
aggregator_circuit<ppT, other_curve>::aggregator_circuit(size_t aggregation_arity):
    aggregation_arity(aggregation_arity)
 {
     ...
     */
    const size_t field_logsize = FieldT::size_in_bits();
    const size_t elt_size = other_field::size_in_bits();
    ...
    const size_t vk_size_in_bits = r1cs_ppzksnark_verification_key_variable<ppT>::size_in_bits(digest_size);
    const size_t proof_size_in_bits = r1cs_ppzksnark_proof_variable<ppT>::size() * field_logsize;
    ...
    for(uint i=0; i<aggregation_arity; i++)
    {
        proofs.emplace_back(r1cs_ppzksnark_proof_variable<ppT>(pb, "proof"));
        proofs_bits[i].allocate(pb, proof_size_in_bits, "Proof in bits");
        proofs_contents[i].allocate(pb, 
            r1cs_ppzksnark_proof_variable<ppT>::size(), 
            "Intermediate variable storing the field representation of the proof"
        );
        unpack_proofs.emplace_back(multipacking_gadget<FieldT>(pb,
            proofs_bits[i],
            proofs_contents[i],
            field_logsize,
            "Unpack the proof"
    ...
    for(uint i=0; i<aggregation_arity; i++)
    {
        ...
        verification_keys.emplace_back(r1cs_ppzksnark_verification_key_variable<ppT>(pb,
     ...
     * 
     */
    block_for_input_verification.reset(new block_variable<FieldT>(pb, 
        block_inputs_vars,
        "Block variable for verification key | proof | verifier input")
    );
    assert(block_for_input_verification->bits.size() == block_size);
    hash_incoming_proof.reset(new CRH_with_field_out_gadget<FieldT>(pb,
        block_size,
...
template<typename ppT, typename other_curve>
void aggregator_circuit<ppT, other_curve>::generate_r1cs_constraints(){
    ...
    {
        pb.add_r1cs_constraint(r1cs_constraint<FieldT>(1, 1 - verification_result, 0), "Enforce valid proof");
    }
...
template<typename ppT, typename other_curve>
void aggregator_circuit<ppT, other_curve>::generate_r1cs_witness(
    ...
    size_t elt_size = other_field::size_in_bits();
    ...
    {
        for(const other_field &val: verifier_input_values[k])
        {
            v = libff::convert_field_element_to_bit_vector<other_field>(val, elt_size);
